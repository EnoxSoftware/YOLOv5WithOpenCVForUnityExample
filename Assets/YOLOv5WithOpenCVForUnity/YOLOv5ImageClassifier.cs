...
{
    /// <summary>
    /// YOLOv5 image classifier implementation.
    /// This class provides functionality for image classification using the YOLOv5 model implemented with OpenCV's DNN module.
    /// Referring to:
    ...
    /// yolov5n-cls.onnx
    /// </summary>
    public class YOLOv5ImageClassifier : ProcessingWorkerBase
    {
        protected static readonly Scalar MEAN = new Scalar(0.485, 0.456, 0.406); // RGB mean
        protected static readonly Scalar STD = new Scalar(0.229, 0.224, 0.225); // RGB standard deviation
        protected static readonly Scalar SIMPLIFIED_MEAN = new Scalar(123.675, 116.28, 103.53);
        protected static readonly double SIMPLIFIED_STD = 1.0 / (255.0 * 0.226);
        ...
        /// <summary>
        /// Initializes a new instance of the YOLOv5ImageClassifier class.
        /// </summary>
        ...
        public YOLOv5ImageClassifier(string modelFilepath, string classesFilepath, Size inputSize,
        ...
        protected virtual Mat PreProcess(IReadOnlyList<Mat> images)
        {
            ...
            // YOLOv5 Image Classifier is scalefactor = 1.0 / 255.0, swapRB = true
            Mat blob;

            if (UseSimplifiedNormalization)
            {
                blob = Dnn.blobFromImages(inputSizeMats, SIMPLIFIED_STD, _inputSize, SIMPLIFIED_MEAN, true, false, CvType.CV_32F); // HWC to NCHW, BGR to RGB
            }
            else
            {
                blob = Dnn.blobFromImages(inputSizeMats, 1.0 / 255.0, _inputSize, SCALAR_0, true, false, CvType.CV_32F); // HWC to NCHW, BGR to RGB
...
#else
                    int requiredResultRowLen = (int)row.total();
                    if (_allResultRowBuffer == null || _allResultRowBuffer.Length < requiredResultRowLen)
                        _allResultRowBuffer = new float[requiredResultRowLen];
                    row.get(0, 0, _allResultRowBuffer);
                    Span<float> data = _allResultRowBuffer.AsSpan(0, requiredResultRowLen);
#endif
