...
using YOLOv5WithOpenCVForUnity.Worker;
...
{
    /// <summary>
    /// YOLOv5 Instance Segmentation Example
    /// Referring to:
    ...
    public class YOLOv5InstanceSegmentationExample : MonoBehaviour
    {
        // Public Fields
        [Header("Output")]
...
#if OPENCV_SENTIS_AVAILABLE
        [Tooltip("When enabled, runs YOLOv5 inference with Sentis (MultiBackendDnn.DNN_BACKEND_UNITY_SENTIS). Inspector paths may stay .onnx; at runtime they are rewritten to .sentis and loaded from StreamingAssets (place a matching .sentis beside the onnx file).")]
        ...
        public string Model = "YOLOv5WithOpenCVForUnityExample/yolov5n-seg.onnx";

        [Tooltip("Optional path to a text file with names of classes to label detected objects.")]
        public string Classes = "YOLOv5WithOpenCVForUnityExample/coco.names";
        ...
        // Private Fields
        private YOLOv5InstanceSegmenter _instanceSegmenter;
        ...
        /// <summary>
        /// <see cref="MatSingleFlightSyncAsyncRunner.TryGetLatestResult(out Mat[])"/> で返る配列のインデックス: 検出結果 <see cref="Mat"/>。
        /// </summary>
        private const int SegmentationResultObjectsIndex = 0;

        /// <summary>
        /// <see cref="MatSingleFlightSyncAsyncRunner.TryGetLatestResult(out Mat[])"/> で返る配列のインデックス: マスク <see cref="Mat"/>。
        /// </summary>
        private const int SegmentationResultMasksIndex = 1;
        ...
        public virtual void OnSourceToMatHelperInitialized()
        {
            ...
            if (_fpsMonitor != null)
            {
                ...
                UpdateFpsMonitorInferenceInfo(_fpsMonitor, _instanceSegmenter, UseAsyncInference);
            }
                ...
                if (_instanceSegmenter != null)
                {
                    ...
                    if (_inferenceRunner != null)
                    {
                        _inferenceRunner.SubmitWork(
                            ...
                            syncWork: m =>
                            {
                                (Mat objects, Mat masks) = _instanceSegmenter.Segment(m, useCopyOutput: true);
                                return new[] { objects, masks };
                            },
                            asyncWork: async m =>
                            {
                                CancellationToken ct = _inferenceRunner.InFlightAsyncWorkCancellationToken;
                                (Mat objects, Mat masks) = await _instanceSegmenter.SegmentTaskAsync(m, ct);
                                return new[] { objects, masks };
                            });

                        if (_inferenceRunner.TryGetLatestResult(out Mat[] segmentation)
                            && segmentation.Length > SegmentationResultMasksIndex)
                        {
                            Mat detectedObjects = segmentation[SegmentationResultObjectsIndex];
                            Mat masks = segmentation[SegmentationResultMasksIndex];
                            _instanceSegmenter.VisualizeMask(rgbaMat, detectedObjects, masks, 0.5f, true);
                            _instanceSegmenter.Visualize(rgbaMat, detectedObjects, false, true);
                        }
                    }
                }
        ...
        private void InitializeInference()
        {
            ...
            try
            {
                ...
                if (UseSentisInference)
                {
                    _instanceSegmenter = new YOLOv5InstanceSegmenter(
                        modelPath,
                        _classesFilepath,
                    ...
                    Debug.Log("YOLOv5InstanceSegmenter initialized (Sentis / DNN_BACKEND_UNITY_SENTIS, backend=" + YoloSentisBackendType + ").");
                }
                else
#endif
                {
                    _instanceSegmenter = new YOLOv5InstanceSegmenter(
                        ...
                        NmsThreshold,
                        TopK);
                    Debug.Log("YOLOv5InstanceSegmenter initialized (OpenCV DNN).");
                }
                ...
                _inferenceRunner = new MatSingleFlightSyncAsyncRunner(
                    ...
                    {
                        await _instanceSegmenter.WaitForCompletionTaskAsync();
                    });
            }
            catch (Exception ex)
            {
                Debug.LogWarning("YOLOv5InstanceSegmentationExample InitializeInference failed: " + ex);
            }
        }
