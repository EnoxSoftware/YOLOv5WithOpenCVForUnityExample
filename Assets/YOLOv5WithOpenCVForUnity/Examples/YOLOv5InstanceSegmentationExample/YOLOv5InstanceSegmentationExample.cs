...
namespace YOLOv5WithOpenCVForUnityExample
{
    ...
    public class YOLOv5InstanceSegmentationExample : MonoBehaviour
    {
        ...
        public string _model = "YOLOv5WithOpenCVForUnityExample/yolov5n-seg.onnx";

        [Tooltip("Optional path to a text file with names of classes to label detected objects.")]
        public string _classes = "YOLOv5WithOpenCVForUnityExample/coco.names";
        ...
        private Mat _latestDetectedMasks;
        ...
        protected virtual void Run()
        {
            ...
            {
                _instanceSegmenter = new YOLOv5InstanceSegmenter(_model_filepath, _classes_filepath, new Size(_inpWidth, _inpHeight), _confThreshold, _nmsThreshold, _topK);
            }
        ...
        public virtual void OnSourceToMatHelperDisposed()
        {
            ...
            _latestDetectedMasks?.Dispose(); _latestDetectedMasks = null;
                    ...
                    if (_useAsyncInference)
                    {
                        ...
                        if (_inferenceTask == null || _inferenceTask.IsCompleted)
                        {
                            ...
                            _inferenceTask = Task.Run(async () =>
                            {
                                try
                                {
                                    // Instance segmenter inference
                                    var (newObjects, newMasks) = await _instanceSegmenter.SegmentAsync(_bgrMatForAsync);
                                    RunOnMainThread(() =>
                                        {
                                            _latestDetectedObjects?.Dispose();
                                            _latestDetectedObjects = newObjects;
                                            _latestDetectedMasks?.Dispose();
                                            _latestDetectedMasks = newMasks;
                                        });
                                }
                        ...
                        if (_latestDetectedObjects != null && _latestDetectedMasks != null)
                        {
                            _instanceSegmenter.VisualizeMask(rgbaMat, _latestDetectedObjects, _latestDetectedMasks, 0.5f, true);
                            _instanceSegmenter.Visualize(rgbaMat, _latestDetectedObjects, false, true);
                        }
                    }
                    else
                    {
                        ...
                        // Instance segmenter inference
                        var (objects, masks) = _instanceSegmenter.Segment(_bgrMat);
                        using (objects)
                        using (masks)
                        {
                            // tm.stop();
                            // Debug.Log("YOLOv5InstanceSegmentator Inference time, ms: " + tm.getTimeMilli());

                            Imgproc.cvtColor(_bgrMat, rgbaMat, Imgproc.COLOR_BGR2RGBA);

                            _instanceSegmenter.VisualizeMask(rgbaMat, objects, masks, 0.5f, true);
                            _instanceSegmenter.Visualize(rgbaMat, objects, false, true);
                        }
                    }
                }
