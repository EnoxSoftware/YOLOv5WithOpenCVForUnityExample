...
namespace YOLOv5WithOpenCVForUnityExample
{
    ...
    public class YOLOv5ImageClassificationExample : MonoBehaviour
    {
        ...
        private Mat _latestClassProbabilities;
        ...
        protected virtual void Run()
        {
            ...
            {
                _imageClassifier = new YOLOv5ImageClassifier(_model_filepath, _classes_filepath, new Size(_inpWidth, _inpHeight));
            }
        ...
        public virtual void OnSourceToMatHelperDisposed()
        {
            ...
            _latestClassProbabilities?.Dispose(); _latestClassProbabilities = null;
            ...
            if (_multiSource2MatHelper.IsPlaying() && _multiSource2MatHelper.DidUpdateThisFrame())
            {
                ...
                else
                {
                    ...
                    if (_useAsyncInference)
                    {
                        ...
                        if (_inferenceTask == null || _inferenceTask.IsCompleted)
                        {
                            ...
                            _inferenceTask = Task.Run(async () =>
                            {
                                try
                                {
                                    ...
                                    var newClassProbabilities = await _imageClassifier.ClassifyAsync(_bgrMatForAsync);
                                    RunOnMainThread(() =>
                                        {
                                            _latestClassProbabilities?.Dispose();
                                            _latestClassProbabilities = newClassProbabilities;
                                        });
                                }
                        ...
                        if (_latestClassProbabilities != null)
                        {
                            _imageClassifier.Visualize(rgbaMat, _latestClassProbabilities, false, true);
                        }
                    }
                    else
                    {
                        ...
                        // Image classifier inference
                        using (Mat classProbabilities = _imageClassifier.Classify(_bgrMat))
                        {
                            // tm.stop();
                            // Debug.Log("YOLOv5ImageClassifier Inference time, ms: " + tm.getTimeMilli());
