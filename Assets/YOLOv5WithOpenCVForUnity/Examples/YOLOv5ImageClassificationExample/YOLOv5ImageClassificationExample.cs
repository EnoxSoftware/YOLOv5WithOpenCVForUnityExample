...
{
    /// <summary>
    /// YOLOv5 Image Classification Example
    /// Referring to:
    ...
    public class YOLOv5ImageClassificationExample : MonoBehaviour
    {
        // Public Fields
        [Header("Output")]
        ...
        public string Model = "YOLOv5WithOpenCVForUnityExample/yolov5n-cls.onnx";

        [Tooltip("Optional path to a text file with names of classes to label detected objects.")]
        public string Classes = "YOLOv5WithOpenCVForUnityExample/imagenet_labels.txt";
        ...
        // Private Fields
        private YOLOv5ImageClassifier _imageClassifier;
        ...
        public virtual void OnSourceToMatHelperInitialized()
        {
            ...
            if (_fpsMonitor != null)
            {
                ...
                UpdateFpsMonitorInferenceInfo(_fpsMonitor, _imageClassifier, UseAsyncInference);
            }
                ...
                if (_imageClassifier != null)
                {
                    ...
                    if (_inferenceRunner != null)
                    {
                        _inferenceRunner.SubmitWork(
                            _bgrMat,
                            syncWork: m => _imageClassifier.Classify(m, useCopyOutput: true),
                            asyncWork: async m =>
                            {
                                CancellationToken ct = _inferenceRunner.InFlightAsyncWorkCancellationToken;
                                return await _imageClassifier.ClassifyTaskAsync(m, ct);
                            });

                        if (_inferenceRunner.TryGetLatestResult(out Mat classProbabilities))
                        {
                            _imageClassifier.Visualize(rgbaMat, classProbabilities, false, true);

                            if (_fpsMonitor != null)
                            {
                                var sortedData = _imageClassifier.GetSortedData(classProbabilities, 5);
        ...
        private void InitializeInference()
        {
            ...
            try
            {
                ...
                if (UseSentisInference)
                {
                    _imageClassifier = new YOLOv5ImageClassifier(
                        modelPath,
                        _classesFilepath,
                        new Size(InpWidth, InpHeight),
                        MultiBackendDnn.DNN_BACKEND_UNITY_SENTIS,
                        (int)YoloSentisBackendType);
                    Debug.Log("YOLOv5ImageClassifier initialized (Sentis / DNN_BACKEND_UNITY_SENTIS, backend=" + YoloSentisBackendType + ").");
                }
                else
#endif
                {
                    _imageClassifier = new YOLOv5ImageClassifier(
                        ...
                        _classesFilepath,
                        new Size(InpWidth, InpHeight));
                    Debug.Log("YOLOv5ImageClassifier initialized (OpenCV DNN).");
                }
                ...
                _inferenceRunner = new MatSingleFlightSyncAsyncRunner(
                    ...
                    {
                        await _imageClassifier.WaitForCompletionTaskAsync();
                    });
            }
            catch (Exception ex)
            {
                Debug.LogWarning("YOLOv5ImageClassificationExample InitializeInference failed: " + ex);
            }
        }
