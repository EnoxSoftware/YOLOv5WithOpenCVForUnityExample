...
using UnityEngine;
using YOLOv5WithOpenCVForUnity.UnityIntegration.Worker;
using YOLOv5WithOpenCVForUnity.UnityIntegration.Worker.DataStruct;
using YOLOv5WithOpenCVForUnity.UnityIntegration.Worker.Utils;
using OpenCVRect = OpenCVForUnity.CoreModule.Rect;

namespace YOLOv5WithOpenCVForUnity.Worker
{
    /// <summary>
    /// YOLOv5 instance segmenter implementation.
    /// This class provides functionality for instance segmentation using the YOLOv5 model implemented with OpenCV's DNN module.
    /// Referring to:
    ...
    /// yolov5n-seg.onnx
    /// </summary>
    public class YOLOv5InstanceSegmenter : ProcessingWorkerBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the YOLOv5InstanceSegmenter class.
        /// </summary>
        ...
        public YOLOv5InstanceSegmenter(string modelFilepath, string classesFilepath, Size inputSize,
        ...
        protected virtual Mat PreProcess(Mat image)
        {
            ...
            // YOLOv5 is scalefactor = 1.0 / 255.0, swapRB = true
        ...
        protected virtual (Mat result, Mat masks) PostProcess(List<Mat> outputBlobs, (double width, double height) originalSize)
        {
            ...
            {
                // pre-NMS
                PreNMS_BoxConfClsconfsMaskcoeff_CXCYWHtoX1Y1X2Y2(outputBlob_NxBoxConfClsconfsMaskcoeff, ref _preNMS_Nx38, _confThreshold, _topK);
            }
        ...
        protected virtual void PreNMS_BoxConfClsconfsMaskcoeff_CXCYWHtoX1Y1X2Y2(Mat boxConfClsconfsMaskcoeff, ref Mat preNMS_Nx38, float score_threshold, int top_k = 300)
        {
            if (!boxConfClsconfsMaskcoeff.isContinuous())
                throw new ArgumentException("boxConfClsconfsMaskcoeff is not continuous.");

            const int CONFIDENCE_INDEX = 4;
            const int CLASS_CONFIDENCES_INDEX = 5;

            int boxConfClsconfsMaskcoeff_rows = boxConfClsconfsMaskcoeff.rows();
            int boxConfClsconfsMaskcoeff_cols = boxConfClsconfsMaskcoeff.cols();
...
#if NET_STANDARD_2_1 && !OPENCV_DONT_USE_UNSAFE_CODE
            ReadOnlySpan<float> allBoxConfClsconfsMaskcoeff = boxConfClsconfsMaskcoeff.AsSpan<float>();
            Span<float> allPreNMS = preNMS_Nx38.AsSpan<float>();
#else
            int requiredBoxConfClsconfsMaskcoeffLen = boxConfClsconfsMaskcoeff_rows * boxConfClsconfsMaskcoeff_cols;
            int requiredPreNMSLen = num_preNMS * (DETECTION_RESULT_COLUMNS + _numMaskCoeff);

            if (_allBoxConfClsconfsBuffer == null || _allBoxConfClsconfsBuffer.Length < requiredBoxConfClsconfsMaskcoeffLen)
                _allBoxConfClsconfsBuffer = new float[requiredBoxConfClsconfsMaskcoeffLen];
            if (_allPreNMSBuffer == null || _allPreNMSBuffer.Length < requiredPreNMSLen)
                _allPreNMSBuffer = new float[requiredPreNMSLen];

            boxConfClsconfsMaskcoeff.get(0, 0, _allBoxConfClsconfsBuffer);
            preNMS_Nx38.get(0, 0, _allPreNMSBuffer);
            ReadOnlySpan<float> allBoxConfClsconfsMaskcoeff = _allBoxConfClsconfsBuffer.AsSpan(0, requiredBoxConfClsconfsMaskcoeffLen);
            Span<float> allPreNMS = _allPreNMSBuffer.AsSpan(0, requiredPreNMSLen);
#endif

            int ind = 0;
            for (int i = 0; i < boxConfClsconfsMaskcoeff_rows; ++i)
            {
                float confidence = allBoxConfClsconfsMaskcoeff[i * boxConfClsconfsMaskcoeff_cols + CONFIDENCE_INDEX];
                if (confidence > score_threshold)
                {
                    ...
                    int maxIdx = -1;
                    int baseIdx = i * boxConfClsconfsMaskcoeff_cols + CLASS_CONFIDENCES_INDEX;
                    for (int j = 0; j < _numClasses; ++j)
                    {
                        float conf = allBoxConfClsconfsMaskcoeff[baseIdx + j];
                        if (conf > maxVal)
                        {
                            maxVal = conf;
                            maxIdx = j;
                    ...
                    int boxIdx = i * boxConfClsconfsMaskcoeff_cols;
                    int preNMSIdx = ind * (DETECTION_RESULT_COLUMNS + _numMaskCoeff);
                    int maskCoeffIdx = i * boxConfClsconfsMaskcoeff_cols + CLASS_CONFIDENCES_INDEX + _numClasses;

                    // Convert from [cx,cy,w,h] to [x1,y1,x2,y2]
                    float cx = allBoxConfClsconfsMaskcoeff[boxIdx];
                    float cy = allBoxConfClsconfsMaskcoeff[boxIdx + 1];
                    float w = allBoxConfClsconfsMaskcoeff[boxIdx + 2];
                    float h = allBoxConfClsconfsMaskcoeff[boxIdx + 3];
                    ...
                    allBoxConfClsconfsMaskcoeff.Slice(maskCoeffIdx, _numMaskCoeff).CopyTo(allPreNMS.Slice(preNMSIdx + DETECTION_RESULT_COLUMNS, _numMaskCoeff));
