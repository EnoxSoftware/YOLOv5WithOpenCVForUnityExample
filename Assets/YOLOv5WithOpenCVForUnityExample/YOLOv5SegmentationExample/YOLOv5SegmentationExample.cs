...
namespace YOLOv5WithOpenCVForUnityExample
{
    ...
    public class YOLOv5SegmentationExample : MonoBehaviour
    {
        ...
        /// <summary>
        /// The YOLOv5 segment predictor.
        /// </summary>
        YOLOv5SegmentPredictor segmentPredictor;
        ...
        private class YOLOv5SegmentPredictor
        {
            Size input_size;
            ...
            public YOLOv5SegmentPredictor(string modelFilepath, string classesFilepath, Size inputSize, float confThreshold = 0.25f, float nmsThreshold = 0.45f, int topK = 1000, bool upsample = true, int backend = Dnn.DNN_BACKEND_OPENCV, int target = Dnn.DNN_TARGET_CPU)
            {
                // initialize
            ...
            protected virtual Mat postprocess(Mat output_blob, Size original_shape)
            {
                ...
                if (output_blob_0.size(2) < 117)
                    return new Mat();

                int nm = 32; // number of masks

                int num = output_blob_0.size(1);
                Mat output_blob_numx117 = output_blob_0.reshape(1, num);
                Mat box_delta = output_blob_numx117.colRange(new OpenCVRange(0, 4));
                Mat confidence = output_blob_numx117.colRange(new OpenCVRange(4, 5));
                Mat classes_scores_delta = output_blob_numx117.colRange(new OpenCVRange(5, 85));
                Mat mask_delta = output_blob_numx117.colRange(new OpenCVRange(85, 85 + nm));
                ...
                Mat results = new Mat(selectedIndices.Count, 6 + nm, CvType.CV_32FC1);
                ...
                for (int i = 0; i < selectedIndices.Count; ++i)
                {
                    ...
                    float[] mask_arr = new float[nm];
                    mask_delta.get(idx, 0, mask_arr);
                    results.put(i, 6, mask_arr);
                }
