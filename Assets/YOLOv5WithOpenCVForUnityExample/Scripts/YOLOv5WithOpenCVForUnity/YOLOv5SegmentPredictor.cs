...
namespace YOLOv5WithOpenCVForUnity
{
    ...
    public class YOLOv5SegmentPredictor : IDisposable
    {
        ...
        public YOLOv5SegmentPredictor(string modelFilepath, string classesFilepath, Size inputSize, float confThreshold = 0.25f, float nmsThreshold = 0.45f, int topK = 300, bool upsample = true, int backend = Dnn.DNN_BACKEND_OPENCV, int target = Dnn.DNN_TARGET_CPU)
        {
            // initialize
        ...
        protected virtual Mat PostProcess(Mat output_blob, Size original_shape)
        {
            ...
            if (output_blob_0.size(2) != 5 + num_classes + num_masks)
            {
                Debug.LogWarning("The number of classes and output shapes are different. " +
                                "( output_blob_0.size(2):" + output_blob_0.size(2) + " != 5 + num_classes:" + num_classes + " + " + num_masks + " )\n" +
                                "When using a custom model, be sure to set the correct number of classes by loading the appropriate custom classesFile.");

                num_classes = output_blob_0.size(2) - 5 - num_masks;
            }
