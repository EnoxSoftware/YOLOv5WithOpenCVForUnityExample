...
namespace YOLOv5WithOpenCVForUnity
{
    ...
    public class YOLOv5SegmentPredictor
    {
        ...
        protected virtual Mat postprocess(Mat output_blob, Size original_shape)
        {
            ...
            if (output_blob_0.size(2) < 5 + num_classes + num_masks)
                return new Mat();
            ...
            Mat classes_scores_delta = output_blob_numx117.colRange(new OpenCVRange(5, 5 + num_classes));
            Mat mask_delta = output_blob_numx117.colRange(new OpenCVRange(5 + num_classes, 5 + num_classes + num_masks));
            ...
            Mat results = new Mat(selectedIndices.Count, 6 + num_masks, CvType.CV_32FC1);
